...
There are many graphics systems in R for making plot and figures.
...
> Q. Can you make the `geom_smooth()` function produce a linear straight line fit to the data and return off the "gray" error region
...
ggplot(genes) +
  ...
  geom_point() + 
  scale_color_manual(values=c("purple", "lightblue1", "cyan3")) +
  theme_minimal() +
...
> Q. How may entries (i.e. rows) are in this dataset?

```{r}
nrow(gapminder)
```

> Q. How many different "countnry" entries are in this dataset?
